...
object Types extends templates.SpecBlogPage {
    ...
    frag(
      s"""
        ...
        Non-value types capture properties of identifiers that
          [are not values](#non-value-types). 
        For example, a [type constructor](#type-constructors) 
          does not directly specify a type of values. 
        However, when a type constructor is applied to the correct type
          arguments, it yields a first-order type, which may be a value type.

        Non-value types are expressed indirectly in Scala. 
        E.g., a method type is described by writing down a method signature, 
          which in itself is not a real type, 
          although it  gives rise to a corresponding [method type](#method-types).
  ...
  def valueTypesSection = Section(
    ...
    List(
      singletonTypesSection,
      literalTypesSection,
      stableTypesSection,
      typeProjectionSection,
      typeDesignatorsSection,
      parameterizedTypesSection,
      tupleTypesSection,
      annotatedTypesSection,
      compoundTypesSection,
      infixTypesSection,
      functionTypesSection,
      existentialTypesSection
    )
  )

  def singletonTypesSection = Section(
    "Singleton Types",
    frag(
      s"""
        ```ebnf
        ...
        A _singleton type_ is of the form ´p.´`type`. Where ´p´ is a path pointing to a
          ...
          value denoted by ´p´ (i.e., the value ´v´ for which `v eq p`). 
        Where the path does not conform to `scala.AnyRef`
          the type denotes the set consisting of only the value denoted by ´p´.
  ...
  def literalTypesSection = Section(
    "Literal Types",
    frag(
      s"""
        ```ebnf
        ...
        A literal type `lit` is a special kind of singleton type which denotes the
          single literal value `lit`. 
        Thus, the type ascription `1: 1` gives the most
        ...
        Concretely, the result of `e.isInstanceOf[lit]` and 
          `e match { case _ : lit => }` is determined by evaluating `e == lit`.

        Literal types are available for all types for which there is dedicated syntax, except `Unit`. 
        This includes the numeric types (other than `Byte` and `Short`
          which don't currently have syntax), `Boolean`, `Char`, `String` and `Symbol`.
      """.md
    )
  )

  def stableTypesSection = Section(
    "Stable Types",
    frag(
      s"""
  ...
  def typeProjectionSection = Section(
    "Type Projection",
    frag(
      s"""
        ```ebnf
  ...
  private val typeDesignatorExamples = List(
    ("t", "ε.type#t"), // (Designator, Expansion)
    ("Int", "scala.type#Int"),
    ("scala.Int", "scala.type#Int"),
    ("data.maintable.Node", "data.maintable.type#Node")
  )

  def typeDesignatorsSection = Section(
    "Type Designators",
    frag(
      s"""
        ```ebnf
        ...
        Specifically, the unqualified type name ´t´ where ´t´ is bound in some
          ...
          ´C.´`this.type#`´t´. 
        If ´t´ is not bound in a class, object, or package, 
          then ´t´ is taken as a shorthand for ε`.type#`´t´.

        A qualified type designator has the form `p.t` where `p` is
          a [path](#paths) and _t_ is a type name. 
      ...
      blockquote(
        ...
        table(cls := "table table-hover")(
          thead(
            tr(th("Designator"), th("Expansion"))
          ),
          tbody(
            typeDesignatorExamples.map {
              case (des, exp) =>
                tr(td(code(des)), td(code(exp)))
            }: _*
          )
        ),
      )
  ...
  def parameterizedTypesSection = Section(
    "Parameterized Types",
    frag(
      s"""
        ```ebnf
        ...
        A _parameterized type_ ´T[ T_1 , \\ldots , T_n ]´ consists of a type
          designator ´T´ and type parameters ´T_1 , \\ldots , T_n´ where ´n \\geq 1´.  
        ´T´ must refer to a type constructor which takes ´n´ type
  ...
  def tupleTypesSection = Section(
    ...
    frag(
      s"""
        ...
        The _n_-ary tuple class and product trait are defined at least as follows in the
  ...
  def annotatedTypesSection = Section(
    "Annotated Types",
    frag(
      s"""
        ```ebnf
  ...
  def compoundTypesSection = Section(
    "Compound Types",
    frag(
      s"""
        ```ebnf
  ...
  def existentialTypesSection = Section(
    "Existential Types",
    frag(
      s"""
        ```ebnf
    ...
    List(existentialTypesSimplificationRulesSection,
         existentialQuantificationOverValuesSection,
         existentialTypesPlaceholderSyntaxSection)
  )

  def existentialTypesSimplificationRulesSection = Section(
  ...
  def existentialQuantificationOverValuesSection = Section(
  ...
  def existentialTypesPlaceholderSyntaxSection = Section(
    ...
    frag(
      s"""
        ...
        If a lower bound clause `>:´\\,L´` is missing,
          `>:´\\,´scala.Nothing` is assumed. 
        If an upper bound clause `<:´\\,U´` is missing,
          `<:´\\,´scala.Any` is assumed.  
        A wildcard type is a shorthand for an existentially quantified type variable, 
          where the existential quantification is implicit.
  ...
  /* Non-Value Types */
  def nonValueTypesSection = Section(
    "Non-Value Types",
    frag(
      s"""
        The types explained in the following do not denote sets of values, nor
          do they appear explicitly in programs. 
        They are introduced in this
          report as the internal types of defined identifiers.
      """.md
    ),
    List(methodTypesSection, polymorphicMethodTypesSection, typeConstructorsSection)
  )

  def methodTypesSection = Section(
    "Method Types",
    frag(
      s"""
        A _method type_ is denoted internally as ´(\\mathit{Ps})U´, where ´(\\mathit{Ps})´
          ...
          for some ´n \\geq 0´ and ´U´ is a (value or method) type.  
        This type represents named methods that take arguments named ´p_1 , \\ldots , p_n´
        ...
        They are written here `=> T`. 
        Parameterless methods name expressions that are re-evaluated 
          each time the parameterless method name is referenced.

        Method types do not exist as types of values. 
        If a method name is used as a value, its type is [implicitly converted](06-expressions.html#implicit-conversions) to a
  ...
  def polymorphicMethodTypesSection = Section(
    ...
    frag(
      s"""
        A polymorphic method type is denoted internally as `[´\\mathit{tps}\\,´]´T´` where
          ...
          (value or method) type.
        This type represents named methods that
  ...
  def typeConstructorsSection = Section(
    "Type Constructors",
    frag(
      s"""
...
define a function `f} which has type `(x: T)T ´\overload´ Int`.
  ...
  def baseTypesMemberDefinitionsSection = Section(
    ...
    frag(
      s"""
        ...
        1. The _member bindings_ of a type ´T´ are
            ...
            1. all bindings of the type's [refinement](#compound-types), if it has one
  ...
  private val reationsBetweenTypes = List(
  ...
  def relationsBetweenTypesSection = Section(
    "Relations between types",
    frag(
      "We define the following relations between types.",
      table(cls := "table table-hover")(
        thead(
          tr(th("Name"), th("Symbolically"), th("Interpretation"))
        ),
        tbody(
          reationsBetweenTypes.map {
            case (name, sym, interp) =>
              tr(td(name), td(s"´$sym´".md), td(interp))
          }: _*
        )
      )
    ),
    List(equivalenceSection, conformanceSection, weakConformanceSection, compatibilitySection)
  )

  def equivalenceSection = Section(
    "Equivalence",
    frag(
      s"""
  ...
  def conformanceSection = Section(
    "Conformance",
    frag(
      s"""
    ...
    List(leastUpperBoundsGreatestLowerBoundsSection)
  )

  def leastUpperBoundsGreatestLowerBoundsSection = Section(
  ...
  def weakConformanceSection = Section(
    "Weak Conformance",
    frag(
      s"""
  ...
  def compatibilitySection = Section(
    "Compatibility",
    frag(
      s"""
        ...
        after applying [eta-expansion](06-expressions.html#eta-expansion). If ´T´ is a method type, it's converted to the corresponding function type. If the types do not weakly conform, the following alternatives are checked in order:
          ...
          - SAM conversion: if ´T´ corresponds to a function type, and ´U´ declares a single abstract method 
            whose type [corresponds](06-expressions.html#sam-conversion) to the function type ´U'´, `´T <:_w U'´`
  ...
  def volatileTypesSection = Section(
    "Volatile Types",
    frag(
      s"""
  ...
  def typeErasureSection = Section(
    "Type Erasure",
    frag(
      s"""
